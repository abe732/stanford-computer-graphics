...
public:
    ...
    //
    // Return a set containing all Vertices which 
    // neighbor the vertex at the provided vertex.
    //
    VertexIndexSet GetNeighboringVertices(size_t idx);
...
private:
    ...
    // A map where keys are vertex indices, and values
    // are sets of vertices neighboring that vertex.
    typedef std::map<size_t, VertexIndexSet> VertexNeighborsMap;
    VertexNeighborsMap mNeighborsMap;

    void updateNeighboringVerticesMapForFace(const Face& face);
    void rebuildNeighboringVerticesMap();
};
